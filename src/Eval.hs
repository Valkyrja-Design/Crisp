...
fileToEval path contents = either (throw . ParseError . show) evalBody $ readCodeFromFile path contents
...
fetchVar x = throw $ InvalidArgument ["expected identifier"] [x]

-- ensure that the argument is an Atom
ensureAtom :: CrispVal -> Eval CrispVal
ensureAtom x@(Atom _) = return x
ensureAtom x = throw $ InvalidArgument ["expected atom"] [x]

-- extract variable name from Atom
extractVar :: CrispVal -> T.Text
extractVar (Atom x) = x
extractVar x = throw $ InvalidArgument ["expected atom"] [x]
